...
class ActivitySlotLinkToActivityMomentTest(MigratorTestCase):
    ...
    def prepare(self):
        ...
        activitymoment = ActivityMoment.objects.create(id=1, parent_activity=activity,
            recurrence_id=dateparse.parse_datetime("2022-01-03T18:00:00Z"))

        activityslot_with_activitymoment = ActivitySlot.objects.create(id=1, title="Has ActivityMoment",
            parent_activity=activity, recurrence_id=self.start_1)
        activityslot_without_activitymoment = ActivitySlot.objects.create(id=2, title="No ActivityMoment",
            parent_activity=activity, recurrence_id=self.start_2)
        activityslot_without_recurrence_id = ActivitySlot.objects.create(id=3, title="No recurrence_id",
            parent_activity=activity, recurrence_id=None)

        # 2nd activity
        activity_2 = Activity.objects.create(id=2, **activity_data)
        activityslot_different_activity_no_id = ActivitySlot.objects.create(id=4, title="xxx",
            parent_activity=activity_2, recurrence_id=None)
        activityslot_different_activity_id = ActivitySlot.objects.create(id=5, title="xxx",
            parent_activity=activity_2, recurrence_id=self.start_1)
