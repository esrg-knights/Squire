...
class GroupPanelAccessAdminTestCase(ViewValidityMixin, TestCase):
    ...
    def setUp(self):
        self.model_admin = GroupPanelAccessAdmin(
    ...
    def test_delete_permission(self):
        self.client.request()
        self.assertEqual(
            self.model_admin.has_delete_permission(self.client.get('').request),
            False
        )
    ...
    def test_succesful_list(self):
        self.assertValidGetResponse(
            url=reverse("admin:committees_associationgrouppanelcontrol_changelist")
        )
