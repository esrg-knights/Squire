...
class SyncHelperTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.planner = SquireWorkspaceGroupPlanner(settings)
        ...
        self.member_proxy = MailingListMemberProxy(5, "proxyname", "test@example.com")
    ...
    def test_name(self):
        ...
        sync.workspace_user = WorkspaceUser(name=WorkspaceUserName("fullname"))
        self.assertEqual(sync.name, "fullname")

        # prioritize member name
        sync.squire_member = self.member_proxy
    ...
    def test_valid_email(self):
        ...
        res = self.planner.calc_sync_status_group_members([], [(self.members[0], self.member_proxy)])
        ...
        self.assertEqual(res[0].squire_member, self.member_proxy)
    ...
    def test_sync_remove(self):
        ...
        res = self.planner.calc_sync_status_group_members([self.members[0]], [])
        self.assertEqual(len(res), 1)
        self.assertEqual(res[0].status, WorkspaceGroupMemberSyncStatus.SYNC_SHOULD_REMOVE)
        self.assertEqual(res[0].wgroup_member, self.members[0])
        self.assertIsNone(res[0].workspace_user)
        self.assertIsNone(res[0].squire_member)
    ...
    def test_sync_update(self):
        ...
        self.members[1].id = 4
        res = self.planner.calc_sync_status_group_members([self.members[1]], [(self.members[0], self.member_proxy)])
        self.assertEqual(len(res), 1)
